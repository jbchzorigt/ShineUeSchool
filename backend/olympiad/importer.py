...
SHEET_CATEGORY = {
    "suragch_vi": Category.G6, "suragch_vii": Category.G7, "suragch_viii": Category.G8,
    "suragch_ix": Category.G9, "suragch_x": Category.G10, "suragch_xi": Category.G11,
    "suragch_xii": Category.G12,
    "bagsh_baga": Category.TEACHER_PRIMARY, "bagsh_dund": Category.TEACHER_SECONDARY,
}
...
def detect_category(sheet_name: str, rows: list[list]) -> str | None:
    ...
    for r in rows[:12]:
        ...
        if cells and cells[0].lower().startswith("ангилал"):
            ...
            if "БАГА" in label:
                return Category.TEACHER_PRIMARY
            if "ДУНД" in label:
                return Category.TEACHER_SECONDARY
    return None


def parse_sheet(name: str, rows: list[list], year: int) -> SheetResult:
    category = detect_category(name, rows)
    label = Category(category).label if category else "Танигдсангүй"
    ...
    for r_i, r in enumerate(rows[hdr_i + 1:], start=hdr_i + 2):
        get = lambda i: r[i] if i is not None and i < len(r) else None  # noqa: E731
...
def parse_workbook(source: str | bytes, year: int | None = None) -> ParsedWorkbook:
    """source: файлын зам эсвэл bytes. year өгөөгүй бол файл дахь огнооноос авна."""
    wb = CalamineWorkbook.from_filelike(BytesIO(source)) if isinstance(source, (bytes, bytearray)) else CalamineWorkbook.from_path(source)
    ...
    sheets = [parse_sheet(name, rows, year) for name, rows in all_rows.items()]
    return ParsedWorkbook(sheets=sheets, detected_date=detected)


def import_workbook(parsed: ParsedWorkbook, year: int, replace: bool = True) -> dict:
    ...
    if replace and categories:
        deleted = Result.objects.filter(year=year, category__in=categories).delete()[0]
    for s in parsed.sheets:
        if not s.rows:
            continue
        for r in s.rows:
            r.year = year
        Result.objects.bulk_create(s.rows)
        created += len(s.rows)
